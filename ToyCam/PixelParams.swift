...
struct Uniforms {
    var pixelSize: Float = 4       // 출력 픽셀 블록 크기 (클수록 더 도트)
    ...
    var greenScreen: Float = 1     // LCD 모드 오버라이드: 0=컬러, 1=GB그린, 2=흑백
    var colorMode: Float = 0       // 필터 컬러 모드 (0=일반, 1=GB그린, 2=세피아, 3=앰버, 4=레드, 5=아이스, 6=반전, 7=모노)
...
final class PixelParams: ObservableObject {
    @Published var pixelSize: Double = 4       // 2 ... 40
    ...
    @Published var lcdMode: Int = (UserDefaults.standard.object(forKey: "lcdMode") as? Int) ?? 1 {
        didSet { UserDefaults.standard.set(lcdMode, forKey: "lcdMode") }
    }                                          // LCD 버튼 사이클: 0=컬러, 1=그린, 2=흑백 (마지막 선택 저장)
    @Published var colorMode: Int = 0          // 필터 컬러 모드
    @Published var zoom: Double = 1            // 1 ... 4
    @Published var indicatorAngle: Double = 0  // 휠 인디케이터 회전 (셰이더와 무관)
    @Published var presetIndex: Int = UserDefaults.standard.integer(forKey: "presetIndex") {
        didSet { UserDefaults.standard.set(presetIndex, forKey: "presetIndex") }
    }                                          // 현재 필터 프리셋 (마지막 선택 저장)
    ...
    func fillUniforms(_ u: inout Uniforms) {
        ...
        u.greenScreen = Float(lcdMode)
        u.colorMode = Float(colorMode)
        u.zoom = Float(zoom)
    }
}
